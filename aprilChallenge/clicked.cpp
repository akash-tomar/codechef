...
int main()
{
	...
	for(long long int t=0;t<test;t++) {
        ...
        long long int num_of_cities;//=s.nextInt();
    			long long int num_of_old_cities;// =s.nextInt();
    			long long int length_of_road_old_city;// = s.nextInt();
    			long long int num_of_new_roads;// =s.nextInt();
    			long long int source;// = s.nextInt();
    	cin>>num_of_cities>>num_of_old_cities>>length_of_road_old_city>>num_of_new_roads>>source;
        struct Graph* graph = createGraph(num_of_cities);
        for(long long int i=0;i<num_of_new_roads;i++) {
    				long long int a;
    				long long int b;
    				long long int c;
    				cin>>a>>b>>c;
    	...
    	for(long long int i=1;i<=num_of_old_cities;i++) {
    				for(long long int j=1;j<=num_of_old_cities;j++) {
    					if(i!=j) {
    						addEdge(graph,i-1,j-1,length_of_road_old_city);
    					}
    				}
    			}
